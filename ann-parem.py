...
# Building classifier with a similar architecture as ann.py
# define build_classifier with arguments so they can be changed using grid search
def build_classifier(optimizer):
    ...
    # Compiling the ANN
    # optimizer= argument given by grid search
    # loss function => logarithmic loss function, dependent variable has a binary outcome so, loss = 'binary_crossentropy'
    classifier.compile(optimizer= optimizer, loss='binary_crossentropy', metrics=['accuracy'])
    return classifier

# Create global classifier batch_size and epochs will be defined by grid search
classifier = KerasClassifier(build_fn= build_classifier)

# Grid search will train the ANN using k-fold cross validation with different combinations of these parameters and return the optimal combination
# Parameters will be a dictionary with the parameter that you want to change with its new values that you want to try
...
grid_search = grid_search.fit(x_train, y_train) #fit ANN

# best selection of hyper parameter and best accuracy
